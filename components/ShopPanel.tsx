...
export const ShopPanel: React.FC<ShopPanelProps> = ({ stats, inventory, onBuy, onSell, onContinue, lang }) => {
  ...
  return (
    <div className="flex flex-col h-full noir-panel rounded-lg p-0 relative overflow-hidden shadow-2xl border border-white/10">
      ...
      {/* Header */}
      <div className="p-6 border-b border-white/10 bg-black/40 flex justify-between items-end">
        <div>
          <h2 className="text-2xl font-serif text-white mb-1 tracking-wide">{t.shopTitle}</h2>
        ...
        <div className="text-right">
           ...
           <div className="font-mono text-xl text-luxury-gold">¥{stats[StatType.WEALTH].toLocaleString()}</div>
        </div>
      </div>
      ...
      {/* Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-1 p-1 bg-white/5 overflow-y-auto flex-1 scrollbar-hide">
        {filteredItems.map((item) => {
          ...
          return (
            <div key={item.id} className="bg-black/80 p-5 flex flex-col justify-between hover:bg-[#151515] transition-colors group relative border border-transparent hover:border-white/10">
              
              {/* Item Info */}
              <div>
                <div className="flex justify-between items-start mb-2">
                  <h3 className="text-gray-200 font-bold text-sm tracking-wide group-hover:text-luxury-gold transition-colors">{name}</h3>
                  <span className={`font-mono text-xs ${canAfford ? 'text-luxury-gold' : 'text-gray-600'}`}>¥{item.cost}</span>
                </div>
                <div className="w-8 h-[1px] bg-white/10 mb-3 group-hover:w-full transition-all duration-500"></div>
                <p className="text-gray-500 text-xs mb-4 leading-relaxed h-10 line-clamp-2">{desc}</p>
              </div>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button
                    ...
                    className={`w-7 h-7 flex items-center justify-center border transition-all
                      ${owned > 0 
                        ? 'border-white/20 hover:bg-red-900/50 hover:border-red-500 text-gray-400 hover:text-white' 
                  ...
                  <button
                    onClick={() => onBuy(item)}
                    disabled={!canAfford}
                    className={`h-7 px-4 text-[10px] font-bold uppercase tracking-widest transition-all 
                      ${canAfford 
                        ? 'bg-white/10 text-white hover:bg-luxury-gold hover:text-black border border-white/10 hover:border-luxury-gold' 
      ...
      <div className="p-4 border-t border-white/10 bg-black/60 backdrop-blur">
        <button
          onClick={onContinue}
          className="w-full py-3 bg-luxury-gold text-black font-bold text-sm tracking-[0.3em] uppercase hover:bg-white transition-colors"
        >
          {t.continue}
        </button>
      </div>
