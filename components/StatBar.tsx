  ...
  return (
    <div className="flex flex-col w-full mb-4 group">
      <div className="flex justify-between items-center mb-1.5 px-0.5">
        <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.2em] font-bold text-gray-500 group-hover:text-gray-300 transition-colors">
          <span className="opacity-70">{icon}</span>
          <span>{label}</span>
        </div>
        <span className={`font-mono text-xs font-bold ${activeSegments < 5 && type !== StatType.WEALTH ? 'text-red-500 animate-pulse' : 'text-gray-300'}`}>
          {Math.round(value)}
          <span className="text-gray-600 text-[10px] ml-1">/ {max}</span>
        </span>
      </div>
